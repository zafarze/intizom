	...
	return (
		<>
            ...
            `}>
				...
				<div className="absolute inset-0 bg-gradient-to-br from-indigo-200/90 via-purple-200/90 to-orange-100/90 dark:bg-zinc-950 dark:from-zinc-950 dark:via-zinc-950 dark:to-zinc-950 backdrop-blur-3xl"></div>

				<div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none dark:opacity-0">
				...
				<div className="relative z-10 flex flex-col h-full p-4">
					...
					<div className="flex flex-col mb-8 mt-2 relative">
						<button
							...
							className="absolute right-0 top-1 p-1.5 rounded-lg text-indigo-900/40 hover:text-indigo-600 hover:bg-white/40 dark:text-zinc-600 dark:hover:text-zinc-300 dark:hover:bg-zinc-800/50 transition-colors z-20"
						>
						...
						<div className={`flex flex-col items-center justify-center transition-all duration-300 ${expanded ? 'mt-2' : 'mt-0'}`}>
							<div className={`rounded-2xl bg-white/40 backdrop-blur-md border border-white/60 shadow-sm flex items-center justify-center transition-all duration-300 dark:bg-zinc-900 dark:border-zinc-800 ${expanded ? 'w-16 h-16 mb-3 p-2' : 'w-12 h-12 mb-0 p-1.5'}`}>
								<img src={logoUrl} alt="Logo" className="w-full h-full object-contain drop-shadow-md dark:drop-shadow-none" />
							</div>
							...
							<div className={`text-center transition-all duration-300 overflow-hidden whitespace-nowrap ${expanded ? 'opacity-100 max-h-10' : 'opacity-0 max-h-0'}`}>
								<h2 className="text-[14px] font-bold text-slate-900 dark:text-zinc-100 tracking-tight">{t('auto.t_136_abdura_moni_om')}</h2>
							</div>
						</div>
					</div>
					...
					<div className="flex-1 overflow-y-auto space-y-4 [&::-webkit-scrollbar]:hidden mt-2">
						<div>
							<p className={`text-[10px] font-bold tracking-[0.2em] text-slate-800/40 dark:text-zinc-500 mb-3 transition-all duration-300 whitespace-nowrap ${expanded ? 'opacity-100 text-left pl-2' : 'opacity-0 h-0 hidden'}`}>
								ОСНОВНОЕ
							</p>
...
function NavItem({ to, icon, label, badge, expanded, onClick }: NavItemProps) {
	return (
		<NavLink onClick={onClick} to={to} className={({ isActive }) => `group relative flex items-center px-3 py-3 rounded-xl transition-all duration-300 ease-out overflow-hidden ${expanded ? 'justify-between' : 'justify-center'} ${isActive ? 'bg-white/60 text-slate-900 dark:bg-zinc-800 dark:text-zinc-50 dark:border-transparent font-bold shadow-[0_8px_20px_rgb(0,0,0,0.06)] dark:shadow-none border border-white/60 backdrop-blur-md translate-x-1' : 'text-slate-700 dark:text-zinc-400 font-medium hover:bg-white/40 dark:hover:bg-zinc-900 hover:text-slate-900 dark:hover:text-zinc-100 hover:translate-x-1 hover:shadow-sm dark:hover:shadow-none'}`} title={!expanded ? label : ""}>
			{({ isActive }) => (
				<>
					...
					<div className="flex items-center relative z-10">
						<div className={`transition-all duration-300 ${isActive ? 'scale-110 text-indigo-600 dark:text-zinc-100 drop-shadow-md dark:drop-shadow-none' : 'text-slate-500 dark:text-zinc-500 group-hover:text-indigo-600 dark:group-hover:text-zinc-100 group-hover:scale-110'} ${!expanded ? 'mx-auto' : ''}`}>{icon}</div>
