...
export default function RulesTab({ data, refresh }: { data: any[], refresh: () => void }) {
	...
	const [formData, setFormData] = useState({ title: '', category: 'A', points_impact: -5 });

	const openModal = (item?: any) => { setEditingId(item?.id || null); setFormData(item ? { title: item.title, category: item.category, points_impact: item.points_impact } : { title: '', category: 'A', points_impact: -5 }); setIsModalOpen(true); };
	...
	return (
		<div className="bg-white/60 backdrop-blur-xl border border-white rounded-[2rem] p-6 shadow-sm">
			...
			<TableTemplate headers={['Категория', 'Название правила', 'Баллы', 'Действия']}>
				{data.map(r => (
