...
export default function StudentsTab({ data, classes, refresh }: { data: any[], classes: any[], refresh: () => void }) {
	...
	const handleDeleteLog = async (logId: number) => {
		setConfirmDialog({
			message: "Вы уверены, что хотите отменить это действие? Записи будут удалены, а баллы ученика пересчитаны.",
			onConfirm: async () => {
				try {
			...
			onConfirm: async () => {
				try {
					...
					const html = `
						<html>
						<head>
							<title>Сброшенные доступы учеников - Интизом</title>
							<style>
								@page { margin: 15mm; }
						...
						<body>
							...
							<div class="footer">Распечатано администратором школы. Храните доступы строго в секрете!</div>
						</body>
						</html>
					`;
					printHtml(html);
					...
					toast.success(res.data.detail);
					setSelectedStudents([]); refresh();
				} catch (err) { toast.error("Ошибка при генерации паролей"); }
				finally { setIsSubmitting(false); }
			}
		});
	};
	...
	const exportSelectedToExcel = () => {
		...
		const wsData = selectedData.map(acc => ({
			'Имя': acc.first_name, 'Фамилия': acc.last_name, 'Класс': acc.class_name || '—',
			'Логин': acc.username || '—', 'Пароль': '•••••• (Скрыт настройками безопасности Django)'
		}));
		const ws = XLSX.utils.json_to_sheet(wsData);
		const wb = XLSX.utils.book_new();
		XLSX.utils.book_append_sheet(wb, ws, "Ученики");
		XLSX.writeFile(wb, "Список_Выбранных_Учеников.xlsx");
	...
	const exportSelectedToPDF = () => {
		...
		const html = `
			<html>
			<head>
				<title>Выбранные ученики - Интизом</title>
				<style>
					body { font-family: 'Segoe UI', Arial, sans-serif; padding: 40px; color: #333; }
					h2 { text-align: center; color: #4f46e5; margin-bottom: 30px; }
					table { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 14px; }
			...
			<body>
				<h2>Система "Интизом" - Выбранные ученики</h2>
				<table>
					<tr><th>ФИО Ученика</th><th>Класс</th><th>Логин</th><th>Пароль</th></tr>
					${selectedData.map(acc => `
						<tr>
							...
							<td>${acc.class_name || '—'}</td>
							<td class="mono">${acc.username || '—'}</td>
							<td class="mono" style="color: #94a3b8; font-size: 11px;">(Скрыто БД)</td>
						</tr>
					`).join('')}
				</table>
				<div class="footer">Распечатано из системы управления дисциплиной. Действующие пароли скрыты из соображений безопасности.</div>
			</body>
			</html>
		`;
		printHtml(html);
	...
	return (
		<div className="flex flex-col lg:flex-row gap-6 items-start animate-in fade-in duration-300">
				...
				<div className="overflow-x-auto min-h-[400px]">
					{paginated.length === 0 ? (
						<div className="flex flex-col items-center justify-center min-h-[350px] text-center px-4 w-full h-full">
							...
							<h3 className="text-xl font-black text-slate-800">Список учеников пуст</h3>
							<p className="text-slate-500 font-medium text-sm mt-3 max-w-xs leading-relaxed">
								{searchQuery ? 'По вашему поисковому запросу ничего не найдено.' : 'Пожалуйста, выберите параллель и класс на панели справа, чтобы начать работу с учениками.'}
							</p>
						</div>
					) : (
							...
							{paginated.map((row, idx) => {
								...
								return (
									<tr key={row.id} className={`border-b border-slate-100 transition-colors ${selectedStudents.includes(row.id) ? 'bg-indigo-50/50' : 'hover:bg-slate-50'}`}>
										...
										<td className="py-4 px-4 font-bold text-[12px] text-slate-400 w-12 text-center">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
										...
										<td className="py-4 px-4">
											<ActionButtons 
												...
												extraButton={
													<button onClick={() => openStudentHistory(row.id)} disabled={loadingHistoryId === row.id} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors disabled:opacity-50" title="Посмотреть историю">
			...
			<div className="w-full lg:w-[280px] shrink-0 bg-white/60 backdrop-blur-xl border border-white rounded-[2rem] p-6 shadow-sm sticky top-6">
				<div className="flex items-center gap-2 border-b border-indigo-100/50 pb-4 mb-5">
					<Filter size={20} className="text-indigo-600" />
					<h3 className="font-bold text-slate-800">Фильтр</h3>
				</div>
				...
				<div className="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-2xl p-4 text-white shadow-sm mb-6 relative overflow-hidden group transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
					...
					<div className="relative z-10">
						...
						<div className="flex items-end gap-1.5 mt-2">
							...
							<span className="text-xs font-bold text-indigo-100 mb-1 tracking-wide">учеников</span>
						</div>
					</div>
				</div>
							...
							<div className="flex flex-wrap gap-2">
								{availableExactClasses.map((exactClass) => (
									<button
										...
										className={`px-3 h-10 min-w-[3rem] rounded-xl font-bold text-sm transition-all border ${selectedLetters.includes(exactClass) ? 'bg-indigo-100 text-indigo-700 border-indigo-300 shadow-sm scale-105' : 'bg-white/50 text-slate-600 border-white hover:bg-white'}`}
									>
										{exactClass}
									</button>
								))}
				...
				{(selectedGrade || selectedLetters.length > 0) && (
					<button onClick={() => { setSelectedGrade(null); setSelectedLetters([]); setCurrentPage(1); }} className="mt-5 text-[12px] font-bold text-red-500 py-2.5 w-full bg-red-50/50 hover:bg-red-50 border border-red-100 rounded-xl transition-all">
						Сбросить фильтры
					</button>
				)}
			</div>
				...
				<div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-800/95 backdrop-blur-xl border border-slate-700 text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-4 z-40 animate-in slide-in-from-bottom-10 w-max max-w-[95%] overflow-x-auto scrollbar-hide">
					<div className="flex items-center gap-4 shrink-0">
						<div className="flex flex-col items-center">
							<span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Выбрано</span>
					...
					<div className="flex items-center gap-2 shrink-0">
						...
						<button onClick={() => setShowBulkModal(true)} className="bg-amber-500 hover:bg-amber-600 px-3 py-2 rounded-lg text-xs font-bold transition-all shadow-md active:scale-95 text-white">Сменить класс</button>
						<button onClick={handleBulkDelete} disabled={isSubmitting} className="bg-red-500 hover:bg-red-600 px-3 py-2 rounded-lg text-xs font-bold transition-all shadow-md active:scale-95 disabled:opacity-50 text-white">Удалить</button>
					</div>
